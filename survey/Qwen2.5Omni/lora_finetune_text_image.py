"""
Qwen2.5-Omni LoRA ファインチューニング (Text-Image)
====================================================
...
class TextImageSFTDataset(Dataset):
    ...
    def __init__(self, df: pd.DataFrame, processor: Qwen2_5OmniProcessor, max_length: int = 3072):
    ...
    def _create_labels(self, input_ids: torch.Tensor, messages: list) -> torch.Tensor:
        ...
        # assistant応答のみのテキストを生成してトークン化し、
        # 末尾からマッチングしてラベルを作成
        # 簡易版: im_start/im_end パターンでassistant部分を特定
...
def parse_args():
    parser = argparse.ArgumentParser(description="Qwen2.5-Omni Text-Image LoRA Fine-tuning")
